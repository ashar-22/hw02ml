...
## Training
  ტესტირებული მოდელები: Logistic Regression, XGBoost, AdaBoost და Random Forest. 
   ...
   Logistic Regression   მოდელით დატრენინგებას ყველაზე მეტი დრო სჭირდებოდა, მიუხედავად იმისა, რომ გამოვიყენე scaling და k_best შევამცირე 50-მდე. ვცადე კორელაციის მატრიცის შემოღებაც, თუმცა ამან ტრენინგის დრო მხოლოდ გაზარდა და შედეგი კი არ გააუმჯობესა. Cleaning-ს აქაც იმავენაირად ვშვრებოდი, როგორც სხვა მოდელების შემთხვევაში: ვშლიდი მაღალ Nan-იან სვეტებს და დანარჩენ Nan-იან სხვეტებს სხვადასხვა მეთოდით ვავსებდი:მოდა/მედიანა/საშუალო. Logistic Regression-ის საუკეთესო შედეგი: 
    train_auc   0.8452162653013676
    valid_auc   0.8408705879368424

   XGBoost   მოდელით დატრენინგება ყველაზე მოსახერხებელი, სწრაფი და მაქსიმალურად ახლო პროგნოზით გამორჩეული იყო. აქაც გავწმინდე დატასეტი, OneHotEncoder-ის საშუალებით შევავსე კატეგორიული სვეტები და ნუმერიქალ სვეტებისთვის კი სხვადასხვა ვარიანტები ვცადე. ასევე შემოვიღე Variance Threshold და გავწმინდე იმ მონაცემებისგან რომელთა variance იყო 0.01-ზე დაბალი. ამ მოდელისთვის ვიყენებდი gridSearchCV-ს(ფიქსირებული cv=3). აქაც გამოვიყენე KBest და კორელაციის მატრიცა. KBest-ისთვის ვცადე k=50, 70, 100, 150. k-ს ზრდა საგრძნობლად აუმჯობესებდა შედეგს, რაც შეეხება კორელაციის მატრიცას, ისიც ისეთივე შედეგს გვაძლევდა, როგორსაც დაბალი k. ვფიქრობ, ასე იმიტომ მოხდა, რომ კორელაციის მატრიცისთვის threshold-ის დაწესება რთული აღმოჩნდა, ვინაიდან გატესტვას ბევრი დრო სჭირდებოდა და გამიჭირდა შესაბამისი ზღვარის პოვნა, 0.05 და 0.1 threshold-მა კი თითქმის ვერ შეძლო სვეტების გასუფთავება. საუკეთესო შედეგი მივიღე k=150-ისთვის:
    train_auc   0.9349695289583454
    valid_auc   0.9214822898654824

   AdaBoost   მოდელის გამოყენებამ საკმაოდ კარგი შედეგი გამოიღო, თუმცა მისი ტრენინგის დრო შედარებით უფრო გრძელია, ვიდრე XGBoost-ის. მე გამოვიყენე GridSearchCV-ს 3-ის ფიქსირებული cross-validation-ით, და ტესტირებისთვის ვიყენებდი სხვადასხვა learning rates და n_estimators-ის კომბინაციებს. ადაპტირებული learning rate (0.05-0.1) და 50_estimators-ის შემდეგ მივიღე კარგი შედეგები. სიმართლე გითხრათ, მართლა ბევრ პარამეტრს ვცდიდი, მაგრამ საბოლოოდ ყველაზე უფრო სტაბილური შედეგები მივიღე სწორედ უფრო დაბალი რაოდენობის estimators-ებით. ამ მოდელისთვის მაინც იყო გარკვეული overfitting-ის ნიშნები, როდესაც უფრო მაღალი n_estimators ვცადე, თუმცა დაბალი რაოდენობის ესტიმატორით მივიღე შემდეგი შედეგი(საკმაოდ ახლოსაა ტრეინისა და ვალიდაციის auc, არ შეინიშნება overfitting):
    train_auc   0.8732837744262881
    valid_auc   0.8705382052458751

   Random Forest   მოდელის ტრენინგის დრო XGBoost-სთან შედარებით მეტი იყო, თუმცა კარგი შედეგები მივიღე, როდესაც გადავწყვიტე გამეზრდა n_estimators და შევეცადე მომეხდინა ოპტიმიზაცია max_depth-ისა და min_samples_split-ის პარამეტრებზე. ვცადე grid search-ით სხვადასხვა ჰიპერპარამეტრების კომბინაციები, თუმცა შედეგების მერყეობა დიდი არ ყოფილა და თანაც გრიდით ძებნას აუარებელი დრო სჭირდებოდა, ამიტომ ვცდილობდი, ხელით გადამეცა სხვადასხვა პარამეტრი და დავკვირვებოდი. Random Forest-ისთვის სკალირება და KBest-ის გამოყენებაც მნიშვნელოვანი იყო, თუმცა შემდეგ ვნახე, რომ უკეთესი შედეგი მივიღე, როდესაც max_depth ვაკონტროლე და tree-ის სიმაღლე შევამცირე. ამასთანავე, გადამწყვეტი გახდა feature selection, რადგან n_estimators-ის გაზრდამ სასურველი შედეგი არ გამოიღო, მაგრამ max_features-ის ცვლილებამ მოიტანა შედეგი:
    train_auc   0.8870979274016031
    valid_auc   0.8773935157865418

  ნებისმიერი მოდელისთვის მიღებული train_auc და valid_auc მნიშვნელობები საკმაოდ ახლოსაა ერთმანეთთან, რისი დარეგულირებაც ჰიპერპარამეტრიცაზიით შევძელი. 
